...
window.addEventListener("load", () => {
  ...
  /// 영화선택메뉴 클릭시 기능구현 정의 ///////
  // 1. 영화선택 메뉴 클릭시 클릭된 li에 클래스"on"을 넣는다
  // 2. 나머지 영화선택 메뉴 li에는 클래스"on"을 지운다
  // -> 결과적으로 css로 구현한 선택메뉴 일어서 있기가 구현됨

  // 이벤트 대상: .mlist ul>li
  // 변경 대상 === 이벤트 대상
  // 이벤트 종류 : click
  // 변경 대상이 여러개이므로 for of문을 사용함
  ...
  for (let x of sml) { // x는 각 li요소
    x.onclick = () => {
      // 모든 요소 클래스 "on" 지우기
      for (let z of sml)
      z.classList.remove("on"); // 지우는 게 먼저 써져야 
